...
def test_instance_host_memory():
    instance = glnext.instance(host_memory=128 * 1024, headless=True, layers=['VK_LAYER_KHRONOS_validation'])
...
def test_instance_device_memory():
    instance = glnext.instance(device_memory=1024, headless=True, layers=['VK_LAYER_KHRONOS_validation'])
...
def test_instance_staging_buffer():
    instance = glnext.instance(staging_buffer=4096, headless=True, layers=['VK_LAYER_KHRONOS_validation'])
    assert instance.device_memory is None
